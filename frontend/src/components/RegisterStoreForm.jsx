...
//#TODO: fix no envia peticion insert, segun backend, crea un insert?
...
const RegisterStoreForm = () => {
	...
	const pronviceOnChange = async (event) => {
		...
		{
			/* Para localidad: 
            https://apis.datos.gob.ar/georef/api/localidades?provincia=Buenos%20Aires&campos=nombre&max=1000 */
		}
	...
	const onSubmit = async (data) => {
		...
		try {
			let registerStoreQuery = await storeRegister(data);
				...
				<Form.Group className="mb-3" controlId="formBasicName">
					...
					<Form.Text className="text-muted">
						{errors.email?.type === "required" && (
							<Alert variant="danger">Es obligatorio escribir el correo</Alert>
						)}
					</Form.Text>
				</Form.Group>
				...
				<Form.Group className="mb-3" controlId="formBasicPassword">
					...
					<Form.Text className="text-muted">
						...
						{errors.password?.type === "minLength" && (
							<Alert variant="danger">Minimo de contraseña: 6 caracteres</Alert>
						)}
